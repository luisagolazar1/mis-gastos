...
function BudgetModal({ budgets, categories, expenses, onSave, onClose, budgetChecked, onSaveChecked }) {
  ...
  // Reference period spending (selected month)
  const refSpending = useMemo(() => {
    if (!expenses?.length) return {};
    const acc = {};
    expenses.filter(e => e.date.startsWith(refMonth))
      .forEach(e => { acc[e.catId] = (acc[e.catId] || 0) + e.amount; });
    return acc;
  }, [refMonth, expenses]);

  const refTotal = useMemo(() => Object.values(refSpending).reduce((s,v) => s+v, 0), [refSpending]);
  ...
  return (
    <Modal title="Presupuesto" onClose={onClose}>
      ...
      {currentSpent > 0 && (
        <div style={{ background: T.accentLt, border: `1px solid ${T.border}`, borderRadius: 10, padding: "8px 12px", marginBottom: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <p style={{ fontSize: 11, color: T.muted, fontWeight: 700, letterSpacing: .8, textTransform: "uppercase", margin: 0 }}>Por categoría (opcional)</p>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 10, color: T.subtle }}>Referencia:</span>
          <select value={refMonth} onChange={e => setRefMonth(e.target.value)}
            style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "4px 8px", fontSize: 11, color: T.text, fontFamily: "inherit", cursor: "pointer" }}>
      ...
      {refTotal > 0 && (
        <div style={{ background: T.accentLt, border: `1px solid ${T.border}`, borderRadius: 10, padding: "8px 12px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 11, color: T.muted }}>💡 Total {refMonth}</span>
          <span style={{ fontSize: 13, fontWeight: 700, color: T.accent }}>{fmt(refTotal)}</span>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "28px 32px 1fr 80px 80px 105px", gap: 4, alignItems: "center", marginBottom: 8, paddingBottom: 8, borderBottom: `1px solid ${T.border}` }}>
        ...
        <span style={{ fontSize: 10, color: T.subtle, fontWeight: 700, textAlign: "center" }}>{refMonth}</span>
        <span style={{ fontSize: 10, color: T.subtle, fontWeight: 700, textAlign: "center" }}>Límite</span>
      </div>

      {categories.map(c => {
        const ref     = refSpending[c.id] || 0;
      ...
      {tab==="next"&&(
        <div style={{animation:"fadeSlideUp .3s ease both"}}>
          <Card style={{marginBottom:12,textAlign:"center"}}>
            ...
            {budgetTotal>0&&(
              <div style={{background:totalAvg>budgetTotal?T.warnLt:T.accentLt,borderRadius:10,padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",border:`1px solid ${totalAvg>budgetTotal?"#f5c6c6":T.border}`}}>
                <span style={{fontSize:12,color:T.muted}}>vs presupuesto ({fmt(budgetTotal)})</span>
                <span style={{fontSize:13,fontWeight:700,color:totalAvg>budgetTotal?T.warn:T.accentMd}}>
                  {totalAvg>budgetTotal?`⚠️ excede en ${fmt(Math.round(totalAvg-budgetTotal))}`:`✅ dentro del límite`}
                </span>
              </div>
            )}
          </Card>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        ...
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <span style={{fontSize:10,color:tp>5?T.warn:tp<-5?T.accentMd:T.subtle,fontWeight:700}}>{tp>5?"↑":tp<-5?"↓":"→"} {tp>0?"+":""}{tp}%</span>
                          <span style={{fontSize:14,fontWeight:700,color:limit>0&&avg>limit?T.warn:T.accent}}>{fmt(Math.round(avg))}</span>
                        </div>
                      </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:16}}>
              {forecast.map(f=>(
                <div key={f.months} style={{background:T.bg,borderRadius:14,padding:"14px",border:`1px solid ${T.border}`,textAlign:"center"}}>
                  ...
                  {budgetTotal>0&&<div style={{fontSize:10,color:T.muted}}>Presupuesto: {fmt(budgetTotal*f.months)}</div>}
                </div>
              ))}
            </div>
            ...
            {(()=>{
              ...
              return(
                <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{display:"block"}}>
                  {meses.map((m,i)=>{
                    const x=pL+i*(iW/meses.length)+2;
                    const bH=m.total>0?(m.total/maxVal)*iH:1;
                    const y=pT+iH-bH;
                    return(
                      <g key={m.m}>
                        <rect x={x} y={y} width={bW} height={bH} fill={m.isProj?`${T.accent}55`:T.accent} rx="2" stroke={m.isProj?T.accent:"none"} strokeDasharray={m.isProj?"3,2":"none"}/>
                        <text x={x+bW/2} y={H-2} textAnchor="middle" fontSize="7" fill={m.isProj?T.accent:T.subtle}>{m.label}</text>
                        {m.total>0&&<text x={x+bW/2} y={y-2} textAnchor="middle" fontSize="6" fill={T.subtle}>{Math.round(m.total/1000)}K</text>}
                      </g>
                    );
                  })}
                  <polyline points={tY.map((y,i)=>{const x=pL+i*(iW/meses.length)+2+bW/2;return`${x},${pT+iH-(y/maxVal)*iH}`;}).join(" ")} fill="none" stroke="#e74c3c" strokeWidth="2" strokeDasharray="5,3" opacity=".85"/>
                </svg>
              );
            })()}
          </Card>
          ...
          <Card style={{marginBottom:12}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
              {[
                ...
                {label:"Simulado",val:fmt(Math.round(simTotal)),color:T.accent,bg:T.accentLt},
                {label:simSaving>0?"Ahorro":"Aumento",val:(simSaving>0?"-":"+")+fmt(Math.abs(Math.round(simSaving))),color:simSaving>0?T.accentMd:T.warn,bg:simSaving>0?"#f0faf0":T.warnLt},
              ].map(k=>(
                <div key={k.label} style={{background:k.bg,borderRadius:12,padding:"10px",textAlign:"center",border:`1px solid ${T.border}`}}>
                  <div style={{fontSize:9,color:T.muted,marginBottom:3,textTransform:"uppercase"}}>{k.label}</div>
                  <div style={{fontSize:14,fontWeight:700,color:k.color}}>{k.val}</div>
                </div>
              ))}
            </div>
            {budgetTotal>0&&(
              <div style={{background:simTotal<=budgetTotal?T.accentLt:T.warnLt,borderRadius:10,padding:"8px 14px",marginBottom:12,border:`1px solid ${simTotal<=budgetTotal?T.border:"#f5c6c6"}`,textAlign:"center"}}>
                <span style={{fontSize:12,color:T.muted}}>
                  {simTotal<=budgetTotal?`✅ Sobraría ${fmt(Math.round(budgetTotal-simTotal))}`:`⚠️ Excede en ${fmt(Math.round(simTotal-budgetTotal))}`}
                </span>
              </div>
            )}
